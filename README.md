...
- **多线程下载**：更新包默认 **4 线程分块**下载（按 `Range` 把包切成 4 段并行拉，每段各写一个 `.part` 临时文件，全部到齐后按顺序合并成一个包），比单线程明显更快；**服务器不支持 Range 或分块中途失败（断流/超时/分段坏掉）会自动改用单线程重下**，并在进度条上写清"已自动改用单线程重新下载"，不会让人看着进度归零发懵；下载中取消就是立刻停，**不会偷偷回退重下**；
- **完整性校验（SHA256）**：边下边算 SHA256，清单里给了 `sha256`（便携包用 `portableSha256`）就比对，对不上**直接丢弃并报错**，绝不把损坏或被篡改的包拿去安装；地址与哈希是成套取的，不会拿安装包的哈希去校验便携包；下载**全程只写 `.part` 临时文件，校验通过才改名成正式文件**，成功、失败、取消都不会留下 `.part` 垃圾；
- 点"稍后/取消"只关掉本次窗口：之后再点"🔄 版本更新"或"🔍 检查更新"仍会弹出更新窗口执行更新（已下好的同一版本不会重复下载）；
- 检查更新为可选功能，离线/检查失败**不影响旧版本任何功能**；
- **发布者发版清单**：① `npm version <新版本>`（安装包名里带的就是它，别手改三处）；② 在 `latest.json` 的 `notes` 写这一版的功能说明；③ `npm run desktop:build` —— **打完包会自动收尾**，不用再手工改清单：算两个包的 SHA256、写上 `url`/`portableUrl`（中文名自动百分号编码）、把上一条挪进 `history`（留 5 条）、校验并补齐下载站 `data/` 里的包、并把清单写入下载站的 `data/latest.json`；④ **把 `latest.json` 提交并推到 GitHub** —— raw 是客户端内置的备用源，也是老客户端唯一的兜底（它们内置的第一个源是上次的隧道域名，已失效），忘了推的表现是老用户静悄悄永远发现不了新版本。

  **下载站目录约定**：`serve.js` 只对外暴露站点下的 `data/` 目录（`/latest.json` 与 `/data/latest.json` 都会被映射到 `data/latest.json`），所以**更新清单和安装包都必须放在 `data/` 里**；站点根目录若还留着一份 `latest.json` 是不生效的，发版脚本会提醒移走 —— 两个同名清单正是"发错版本"最常见的来源。

  哈希必须和"实际传上去的那个包"对得上，否则所有用户的更新都会被判成校验失败。所以**一律由脚本按本地刚打出来的包生成，不要手工粘贴**，也别去算下载站上的副本（万一上传出岔子，算服务端副本等于把坏文件"盖章认证"了）。脚本会拦住两类致命错误：两个包哈希相同（说明算成了同一个文件）、地址解出来不是这两个包名；并且连"同名但内容不一样"的包也能查出来（比只看体积严）。

  只想单独收尾或校验：`npm run stamp -- --site D:\DawnDrama` / `npm run stamp -- --check`；临时打个测试包不盖清单：`AGNES_SKIP_STAMP=1 npm run desktop:build`；下载站换路径：`AGNES_SITE_DIR=<路径>`，不想自动复制包过去：`AGNES_NO_SITE_COPY=1`。
...
├── update-downloader.js     应用内更新下载（4 线程分块/回退单线程/进度/超时/取消 + SHA256 校验）
├── stamp-latest.js          发版收尾：算两个包的 SHA256 并盖上更新清单（打包时自动调用）
├── build-after.js           electron-builder 打包钩子：打完包自动跑一次发版收尾
